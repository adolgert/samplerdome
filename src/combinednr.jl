...
function reset!(nr::CombinedNextReaction{K,T}) where {K,T}
    # MutableBinaryHeap doesn't support empty!, so create a new one
    nr.firing_queue = MutableBinaryHeap{OrderedSample{K,T},Base.ForwardOrdering}()
...
function next(nr::CombinedNextReaction{K,T}, when::T, rng::AbstractRNG) where {K,T}
    if !isempty(nr.firing_queue)
        ...
        entry = nr.transition_entry[least.clock]
        nr.transition_entry[least.clock] = NRTransition{T}(
            entry.heap_handle, get_survival_zero(entry.distribution),
            entry.distribution, entry.te, entry.t0
        )
        return (least.time, least.clock)
    else
